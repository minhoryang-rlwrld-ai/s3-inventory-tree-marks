...
| 판단 근거 | 비어 있습니다. 선택 사항입니다. |

이슈 폼은 필드 id를 질의 문자열로 받아 값을 미리 채웁니다. 뷰어는 `?template=mark.yml&title=...&path=...&state=...` 형태로 주소를 만듭니다.

판단은 `/deletable` 같은 **명령을 적는 입력 필드**입니다. 폼이 만들어 낸 본문에서 워크플로가 그 명령을 읽어 라벨을 붙입니다. 라벨을 URL 파라미터로 직접 주지 않는 이유는 저장소에 쓰기 권한이 없는 사람은 그 방식으로 라벨을 달 수 없기 때문입니다.

드롭다운이 아니라 입력 필드인 이유는 **GitHub 이슈 폼이 드롭다운을 주소로 미리 고르지 못하기 때문입니다.** 선택지 문구와 0부터 세는 순번을 모두 시험했지만 둘 다 채워지지 않았고, 입력 필드는 정상으로 채워집니다.

자유 입력이라 오타가 날 수 있습니다. 무언가 적혀 있는데 아는 명령이 아니면 집계가 상태를 정하지 않고 `needs-triage`로 넘기며, 유효한 값이 무엇인지 함께 적습니다.
...
판단 필드는 판단 근거보다 **앞에** 있어야 합니다. 본문에서 처음 만나는 명령을 상태로 읽기 때문에, 근거 문장에 우연히 들어간 명령이 판단 값을 덮지 않게 하기 위해서입니다.
